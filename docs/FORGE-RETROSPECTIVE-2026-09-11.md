...
- PR #2's corrected candidate passed both protected checks, merged as 8aa6cf1, and passed main qualification and GHCR publication. A subsequent user-visible Netrunner preview exposed a distinct gap: the page's successful response did not prove its versioned assets were readable. PR #3 repairs that permission mismatch and adds decoded browser asset checks plus public-origin asset evidence to the launch preflight. Pilot approval now requires that evidence, including for previously recorded healthy-server reports.
