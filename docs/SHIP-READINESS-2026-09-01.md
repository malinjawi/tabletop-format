...
| XLSX | Only indirectly supported through other tools | P0 | Import workbook tabs through the same mapping model; never require conversion to CSV first. |
...
| PnPInk | Experimental | P1 | Keep behind an experimental badge until its pinned format and round-trip gate pass. |
| Squib | Missing | P1 high value | Export canonical CSV/XLSX plus `layout.yml`/starter Ruby; import data and declared layout subset with an explicit fidelity report. Squib already separates spreadsheet data, YAML layout, and generated PNG/PDF/SVG ([official docs](https://squib.rocks/)). |
...
| Tabletop Playground | Missing | P1 high value | Export a package with stable object-template GUIDs, saved table state, textures, and manifest. Its packages are structured JSON/templates/assets and game states reference template IDs ([official package docs](https://tabletop-playground.com/knowledge-base/packages/)). |
...
| The Game Crafter | Missing | P1 | First produce a TGC preflight ZIP of individual 300-DPI PNGs with component manifest; direct API publishing follows. TGC requires individual PNG/JPG files, exact templates, safe zones, and bleed ([official card guide](https://help.thegamecrafter.com/article/399-how-to-make-a-card-game-tarot-deck), [templates](https://help.thegamecrafter.com/article/39-templates)). |
...
- [ ] TTS output passes an automated preflight and opens as a setup-ready exact version.
