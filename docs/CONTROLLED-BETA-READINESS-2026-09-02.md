...
Candidate commit: `0f119ec49e496cbe8593594a64b5781cd39a39ee`

Qualified local image: `sha256:a6c74bea7d041f3a426ce15b73efbdd5af4f9b7ea4fd3f86cbc446f461b80564`
...
| Self-serve Sheets/CSV onboarding | Browser smoke created an owned two-card game from CSV, added artwork, captured creator/license/redistribution before upload, and repaired a fail-closed legacy rights blocker without direct API use. The real sidebar now withholds Attach until a public endpoint, identity, target game, mapping, and commit access pass. It rejects localhost before credentials are sent, explains dead tunnels, preserves a user-private resumable proof, rolls shared Sheet properties back after failed attachment, clears the password field, links back to the exact game, and always returns from Connection to the candidate. The real `Code.gs` passes 37 assertions; Chrome exercises the visible failure and success states. | **CSV and connector UX PASS locally; Sheets deployment incomplete.** Google-hosted Apps Script still needs the final stable HTTPS origin and operator-owned deployment. Manual bound-script copying is not consumer self-service, and the connector has not completed its live journey against this exact deployed candidate. |
...
- 183 functional integration tests, including 37 assertions through the real
...
```text
PRODUCTION IMAGE QUALIFIED — sha256:a6c74bea7d041f3a426ce15b73efbdd5af4f9b7ea4fd3f86cbc446f461b80564 records 0f119ec49e496cbe8593594a64b5781cd39a39ee and reproduced the frozen release after restore.
```
...
5. An operator-owned Google Apps Script/Cloud project for the private connector.
   The pilot may use an explicitly distributed bound script; Marketplace
   publication and OAuth verification can remain post-pilot. The final live
   connector must target the stable Forge origin, not a disposable tunnel.
