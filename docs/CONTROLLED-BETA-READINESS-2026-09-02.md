...
Candidate commit: `f228450fb8c80d2e896512d912cafd8f024ab6b4`

Qualified local image: `sha256:d8354126e23b1cee3365029084df09585170e3607b9f52baa5d4f6a9f3aa2f8d`
...
|---|---|---|
| Reproducible and maintainable application | `.github/workflows/quality.yml` defines a SHA-pinned two-leg gate. `deploy/qualified-images.env` pins six multi-platform dependencies. `npm run test:production-image` built the source-labelled candidate and passed 93 real Forgejo/PostgreSQL/S3 journey assertions plus a 53-check fresh-service restore. The strict clean-tree `launch-gate.sh` completed, and the production dependency audit reports zero vulnerabilities. | **Software PASS; operating evidence missing.** The candidate and this audit remain local, so hosted CI has not run them and no registry digest has been promoted. |
...
- 94 local plus 94 protocol collaboration assertions, with 93 against real Forgejo in the production-image drill;
- 65 Store-2 contracts on SQLite and the same 65 on digest-pinned PostgreSQL 16,
...
- one real narrow-browser account-access flow proving conspicuous unchecked
  acceptance, exact policy snapshots, invitation registration, credential
...
- 53 post-restore checks, including retained account-policy receipts, the real restored Git remote and exact PnP, press/A4/Letter, TTC, TTS,
  portable-project, card-sheet, back, and rights-receipt bytes.

The final line was:

```text
PRODUCTION IMAGE QUALIFIED — sha256:d8354126e23b1cee3365029084df09585170e3607b9f52baa5d4f6a9f3aa2f8d records f228450fb8c80d2e896512d912cafd8f024ab6b4 and reproduced the frozen release after restore.
```
