...
Studio text, rich-text, body, badge and pip elements expose **Show value from** with human-readable field names. The version-1 SVG contract adds optional `data-forge-content` metadata for bounded `src`/`text` edits. Baselines track these properties for three-way conflict detection. Sources are limited to core card fields, declared `card.attributes.*` fields, and printing flavor text, artist, or collector number; arbitrary expressions and URLs cannot be introduced through this control. Existing expressions remain untouched when no connection edit is requested. Older SVG working copies do not reset connections. Static text is limited to 5000 characters.
